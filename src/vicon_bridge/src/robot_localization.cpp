...
//功能:  test if have input or not
static int get_char() //define a integer static local varibles function
{
    fd_set rfds;//define a fd_set type
    struct timeval tv;//struct timeval结构体,该结构体是Linux系统中定义
    int ch = 0;

    FD_ZERO(&rfds);//Empty the port set,make the set does not contain any fd*,将set的所有位置0
    FD_SET(0, &rfds);//set the port set,add 0 to set of rfds,将set的  第0位  置1
    tv.tv_sec = 0;//tv_sec为Epoch到创建struct timeval时的秒数
    tv.tv_usec = 10; //设置等待超时时间,,,,,,,,tv_usec为微秒数，即秒后面的零头

    //检测键盘是否有输入
    //先调用宏FD_ZERO将指定的fd_set清零，然后调用宏FD_SET将需要测试的fd加入fd_set，接着调用函数select测试fd_set中的所有fd，最后用宏FD_ISSET检查某个fd在函数select调用后，相应位是否仍然为1。
    
    if (select(1, &rfds, NULL, NULL, &tv) > 0)//select://1:需要检查的文件描述字个数(即检查到fd_set的第几位)，
    {
        ch = getchar(); 
    }
...
void iterate(vector<float>& cam_ee_trans, vector<float>& base_ee_trans, MatrixXf& left, MatrixXf& right)
{
    int rows = 3;
    int cols = 12;
    MatrixXf A = MatrixXf::Zero(rows, cols);//define a all-zero matrix
    ...
    Eigen::Vector3f B;//Vector define a vector,a float 3-dimension vector
    B << cam_ee_trans[0], cam_ee_trans[1], cam_ee_trans[2];

    MatrixXf l_component(12, 12); //Matrix define a matrix,12*12 double float matrix
    Eigen::VectorXf r_component;//define a float x-dimension
...
{
    if(left.fullPivLu().isInvertible())//solving linear equations,whether this matrix is inverted or not
    {
...
bool getOffset(MatrixXf& solution_offset, tf::TransformListener& marker_position_listener)
{
    ...
    try
    {
        marker_position_listener.lookupTransform("vicon/marker_5/marker_5", "ee_link", ros::Time(0), offset_transform);             //broadcast the transform
    }
...
bool saveSolution(string filename, MatrixXf offset, tf::TransformListener& marker_position_listener)
{
    ...
    if(outStream)
    {
        ...
        marker_position_listener.lookupTransform("vicon/marker_0/marker_0", "base_link", ros::Time(0), desk_base_transform);
        marker_position_listener.lookupTransform("vicon/marker_0/marker_0", "vicon/marker_5/marker_5", ros::Time(0), desk_marker5_transform);
        marker_position_listener.lookupTransform("vicon/marker_0/marker_0", "ee_link", ros::Time(0), desk_ee_link_transform);
        }
...
{
    static string solution_save_path = "/home/eric/catkin_ws/src/vicon_bridge/robot_localization/solutions/";
    
    system(STTY_US TTY_PATH);//do not press the enter and read the keyvalue 
    ...
    while(nh.ok())
    {
        ...
        if(solutionAvailable)
        {
            ...
            robot_localtion_broadcaster.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", "world"));
        ...
        if (key == 32) //space_pressed
        {
            ...
            while(data_count < 10)
            {
                ...
                try
                {
                    marker_position_listener.lookupTransform("world","vicon/marker_5/marker_5", ros::Time(0), cam_ee_transform);
